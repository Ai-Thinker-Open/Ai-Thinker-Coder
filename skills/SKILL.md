...
编译成功后必须进行代码审查，确保代码格式规范、逻辑正确、注释清晰：

| 文档 | 说明 |
|------|------|
| [code-review.md](./references/code-review.md) | BL602 项目代码审查规范（格式审查、业务逻辑、axk 函数头、注释质量、审查流程、自动脚本、核对清单） |

**审查时机**：`make` / `bflb_mkimg` 编译成功后必须完成代码审查，通过后方可提交代码。

---

## 编码规范

安信可科技 C 语言编码规范（axk 函数头标准），适用于所有嵌入式产品开发：

| 文档 | 说明 |
|------|------|
| [coding-standard.md](./references/coding-standard.md) | 完整编码规范（axk 函数头、命名规范、安全性、可维护性、文件组织、核对清单） |

**核心要求速览**：
- 所有函数必须以 `axk_` 开头，并附标准函数头注释
- 全局变量 `g_axk_` / 静态变量 `s_axk_` / 宏 `AXK_`
- 4 空格缩进，if/for/while 单行也必须加 `{}`
- 禁止动态内存滥用，禁止无意义注释
...
本 skill 是安信可模组开发的入口指南。使用 `/skill Ai-Thinker-Coder-<芯片型号>` 加载对应芯片的详细开发文档。
